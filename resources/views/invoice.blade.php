...
<script>
    ...
    $('#add-cc').click(function () {
        var i = $(document).find('.credit_card').length;
        var item = '<div class="col-xs-12 item-row margin-top-5">'

            +'<div class="col-md-4">'
            +'<div class="row">'
            +'<div class="form-group">'
            +'<label class="control-label hidden-md hidden-lg">@lang('modules.invoices.creditcard')</label>'
            +'<div class="input-group">'
            +'<div class="input-group-addon"><span class="ui-icon ui-icon-arrowthick-2-n-s"></span></div>'
            +'<input type="number" class="form-control credit_card" name="credit_card[]" >'
            +'</div>'
            +'</div>'
            ...
            +'<input type="number" min="1" class="form-control amount" value="0" name="creditcard_amount[]" >'
            +'</div>'
    ...
    $('#storePayments').on('keyup change','.amount', function () {
    ...
    function calculateTotal()
    {
        ...
        $('.total-field').val(totalAmount.toFixed(2));
    }
    ...
    $('#save-form').click(function(){
        ...
        if(parseFloat(amount) != parseFloat(total)){
            $.toast({
                heading: 'Error',
                text: "{{ __('messages.totalAmountNotMatch') }}",
                position: 'top-right',
                loaderBg:'#ff6849',
                icon: 'error',
                hideAfter: 3500
