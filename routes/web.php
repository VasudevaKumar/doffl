...
Route::group(
	...
	Route::post('/resend', 'RegisterController@resend')->name('resend-verification');
	Route::get('/thank-you', 'RegisterController@thankyou')->name('thank-you');
    ...
    Route::get('language/{lang}', ['as' => 'language.lang', 'uses' => 'HomeController@changeLanguage']);
	    ...
	    //File SHaring	
	    Route::get('/access-file/{token}', ['uses' => 'HomeController@access_file'])->name('access-file');	
	    Route::post('/check-pword/{token}', ['uses' => 'HomeController@check_pword'])->name('check-pword');	
	    Route::post('/download-file/{token}', ['uses' => 'HomeController@download_file'])->name('download-file');	
	    Route::get('/link/{token}', ['uses' => 'HomeController@access_url'])->name('access-link');	
	    	
	    Route::post('/push-delivered', ['uses' => 'HomeController@PushDelivered'])->name('push-delivered');	
	    Route::post('/push-clicked', ['uses' => 'HomeController@PushClicked'])->name('push-clicked');
	    
	    //Widgets
	     Route::post('/widgets', ['uses' => 'HomeController@widgets'])->name('load-widgets');	
	     
	     //Live Chat UserSide Routes
	Route::get('/live-chat/{com_tkn}', 'HomeController@live_chat')->name('live-chat');	
	Route::post('/live-chat/{com_tkn}', 'HomeController@start_live_chat')->name('live-chat-start');	
	Route::get('/live-chat/{com_tkn}/{token}', 'HomeController@live_chat_board')->name('live-chat-board');
	Route::post('/send-msg', 'HomeController@send_msg')->name('chat_send_msg');	
	...
	// Live Chat unnecessary Right Now
	Route::post('/get_chat_data', 'ChatController@validate_domain')->name('validate_domain');	
	Route::post('/setChatUserId', 'ChatController@set_userid')->name('set-chat-user-id');	
	Route::get('/validate-userid', 'ChatController@validate_user')->name('validate_user');	
	Route::post('/check-banned', 'ChatController@check_banned')->name('check-banned');	
	Route::post('/check-ip-banned', 'ChatController@check_ip_banned')->name('check-ip-banned');	
	Route::post('/check-user-banned', 'ChatController@check_user_banned')->name('check-user-banned');	
	Route::post('/create-chat-session', 'ChatController@create_chat_session')->name('create-chat-session');	
	Route::post('/validate-chat-session', 'ChatController@validate_chat_session')->name('validate-chat-session');	

	
		
	//PushNotifications Routes	
	Route::get('/push-config/{token}', 'PushNotifications@config')->name('push-config');	
	Route::get('/push-subscribe/{token}', 'PushNotifications@open')->name('push-open');	
	Route::post('/push-subscribe/{token}', 'PushNotifications@subscribe')->name('push-subscribe');
...
Route::group(['middleware' => 'auth'], function () {
    ...
    Route::group(
        ...
        Route::get('invoices/generate', ['uses' => 'SuperAdminInvoiceController@generate'])->name('invoices.generate');
        ...
        Route::post('/get-invoice', ['uses' => 'SuperAdminInvoiceController@loadInvoice'])->name('invoices.loadInvoice');
    ...
    Route::group(
        ...
        function () {
            Route::group(['middleware' => ['account-setup', 'license-expire']], function () {
                ...
                // knowledgebase
                Route::get('knowledgebase', ['uses' => 'ManageClientCategoryController@index'])->name('knowledgebase')->middleware('kb-limit');

                
                Route::get('knowledgebase/createcategory', ['uses' => 'ManageClientCategoryController@createcategory'])->name('knowledgebase.createcategory')->middleware('kb-limit');

                Route::post('knowledgebase/saveCategory', ['uses' => 'ManageClientCategoryController@saveCategory'])->name('knowledgebase.saveCategory')->middleware('kb-limit');
                
                Route::get('knowledgebase/editCategory/{id}', ['uses' => 'ManageClientCategoryController@editCategory'])->name('knowledgebase.editCategory')->middleware('kb-limit');

                Route::post('knowledgebase/updateCategory', ['uses' => 'ManageClientCategoryController@updateCategory'])->name('knowledgebase.updateCategory')->middleware('kb-limit');


                Route::post('knowledgebase/deleteCategory', ['uses' => 'ManageClientCategoryController@deleteCategory'])->name('knowledgebase.deleteCategory')->middleware('kb-limit');

                Route::get('knowledgebase/manageArticles/{id}', ['uses' => 'ManageClientCategoryController@manageArticles'])->name('knowledgebase.manageArticles')->middleware('kb-limit');

                Route::get('knowledgebase/createArticle/{id}', ['uses' => 'ManageClientCategoryController@createArticle'])->name('knowledgebase.createArticle')->middleware('kb-limit');
                
                Route::post('knowledgebase/saveArticle', ['uses' => 'ManageClientCategoryController@saveArticle'])->name('knowledgebase.saveArticle')->middleware('kb-limit');

                

                Route::get('knowledgebase/editArticle/{articleID}', ['uses' => 'ManageClientCategoryController@editArticle'])->name('knowledgebase.editArticle')->middleware('kb-limit');

                Route::post('knowledgebase/updateArticle', ['uses' => 'ManageClientCategoryController@updateArticle'])->name('knowledgebase.updateArticle')->middleware('kb-limit');
                
                Route::post('knowledgebase/deleteArticle', ['uses' => 'ManageClientCategoryController@deleteArticle'])->name('knowledgebase.deleteArticle')->middleware('kb-limit');
                
                
                // /site settings

                Route::get('settings/knowledgebase', ['uses' => 'ManageClientCategoryController@siteSetting'])->name('knowledgebase.siteSetting');

                Route::post('settings/knowledgebase/saveSetting', ['uses' => 'ManageClientCategoryController@saveSetting'])->name('knowledgebase.siteSetting.saveSetting');

                Route::post('settings/knowledgebase/updateSetting', ['uses' => 'ManageClientCategoryController@updateSetting'])->name('knowledgebase.siteSetting.updateSetting');


                // Recruit

                 Route::get('recruit', ['uses' => 'AdminRecruitController@index'])->name('recruit')->middleware('recruit-limit');

                 Route::post('recruit/saveJob', ['uses' => 'AdminRecruitController@saveJob'])->name('recruit.saveJob')->middleware('recruit-limit');
                 Route::get('recruit/managejob', ['uses' => 'AdminRecruitController@managejob'])->name('recruit.managejob')->middleware('recruit-limit');
                 Route::get('recruit/editJob/{articleID}', ['uses' => 'AdminRecruitController@editJob'])->name('recruit.editJob')->middleware('recruit-limit');

                
                Route::post('recruit/updateJob', ['uses' => 'AdminRecruitController@updateJob'])->name('recruit.updateJob')->middleware('recruit-limit');

                Route::post('recruit/deleteJob', ['uses' => 'AdminRecruitController@deleteJob'])->name('recruit.deleteJob')->middleware('recruit-limit');

                

                Route::get('recruit/manageQuestions/{id}', ['uses' => 'AdminRecruitController@manageQuestions'])->name('recruit.manageQuestions')->middleware('recruit-limit');


                Route::get('recruit/createQuestion/{id}', ['uses' => 'AdminRecruitController@createQuestion'])->name('recruit.createQuestion')->middleware('recruit-limit');
                
                 Route::post('recruit/saveQuestion', ['uses' => 'AdminRecruitController@saveQuestion'])->name('recruit.saveQuestion')->middleware('recruit-limit');
                

                 Route::get('recruit/profilelist/{id}', ['uses' => 'AdminRecruitController@profilelist'])->name('recruit.profilelist')->middleware('recruit-limit');


                    Route::get('recruit/profileview/{id}', ['uses' => 'AdminRecruitController@profileview'])->name('recruit.profileview')->middleware('recruit-limit');
                    
                    Route::get('settings/mediaLinks', ['uses' => 'AdminRecruitController@mediaLinks'])->name('settings.mediaLinks');

                    Route::post('settings/saveMedialinks', ['uses' => 'AdminRecruitController@saveMedialinks'])->name('recruit.settings.saveMedialinks');

                    Route::post('settings/updateMedialinks', ['uses' => 'AdminRecruitController@updateMedialinks'])->name('recruit.settings.updateMedialinks');


                   //  Route::get('urlshortener', ['uses' => 'UrlshortenerController@index'])->name('urlshortener')->middleware('shorturls-limit');

                    Route::get('urlshortener', ['uses' => 'UrlshortenerController@index'])->name('urlshortener');
                    

                    Route::get('bookings', ['uses' => 'BookingsController@index'])->name('bookings')->middleware('bookings-limit');
                    Route::get('addBooking', ['uses' => 'BookingsController@addBooking'])->name('addBooking')->middleware('bookings-limit');
                    Route::post('saveBooking', ['uses' => 'BookingsController@saveBooking'])->name('saveBooking')->middleware('bookings-limit');
                    Route::get('editBooking/{id}', ['uses' => 'BookingsController@editBooking'])->name('editBooking')->middleware('bookings-limit');
                    Route::post('updateBooking', ['uses' => 'BookingsController@updateBooking'])->name('updateBooking')->middleware('bookings-limit'); 
                    Route::post('deleteBooking', ['uses' => 'BookingsController@deleteBooking'])->name('deleteBooking')->middleware('bookings-limit');
                    Route::get('viewbooking/{id}', ['uses' => 'BookingsController@viewbooking'])->name('viewbooking')->middleware('bookings-limit');
                    
                    Route::post('deleteSlot', ['uses' => 'BookingsController@deleteSlot'])->name('deleteSlot')->middleware('bookings-limit'); 

                   Route::get('settings/bookings', ['uses' => 'BookingsController@siteSetting'])->name('bookings.siteSetting');

                    Route::post('settings/bookings/saveSetting', ['uses' => 'BookingsController@saveSetting'])->name('bookings.siteSetting.saveSetting');

                Route::post('settings/bookings/updateSetting', ['uses' => 'BookingsController@updateSetting'])->name('bookings.siteSetting.updateSetting');

                
                 // Payment Links

                    Route::get('paymentLinks', ['uses' => 'PaymentLinkController@index'])->name('paymentLinks')->middleware('payments-limit');
                    Route::get('addPaymentLink', ['uses' => 'PaymentLinkController@addPaymentLink'])->name('addPaymentLink')->middleware('payments-limit');
                    Route::post('savePaymentLink', ['uses' => 'PaymentLinkController@savePaymentLink'])->name('savePaymentLink')->middleware('payments-limit');
                    Route::get('editPaymentLink/{id}', ['uses' => 'PaymentLinkController@editPaymentLink'])->name('editPaymentLink')->middleware('payments-limit');
                    Route::post('updatePaymentLink', ['uses' => 'PaymentLinkController@updatePaymentLink'])->name('updatePaymentLink')->middleware('payments-limit'); 

                    Route::post('deletepaymentLink', ['uses' => 'PaymentLinkController@deletepaymentLink'])->name('deletepaymentLink')->middleware('payments-limit'); 

                    Route::get('viewpaymentLink/{id}', ['uses' => 'PaymentLinkController@viewpaymentLink'])->name('viewpaymentLink')->middleware('payments-limit'); 



                Route::get('settings/paymentlinks', ['uses' => 'PaymentLinkController@siteSetting'])->name('paymentlinks.siteSetting');

                Route::post('settings/paymentlinks/saveSetting', ['uses' => 'PaymentLinkController@saveSetting'])->name('paymentlinks.siteSetting.saveSetting');

                Route::post('settings/paymentlinks/updateSetting', ['uses' => 'PaymentLinkController@updateSetting'])->name('paymentlinks.siteSetting.updateSetting');
...
Route::post('settings/paymentlinks/saveSetting', ['uses' => 'PaymentLinkController@saveSetting'])->name('paymentlinks.siteSetting.saveSetting');
                ...
                Route::group(
                    ...
                    function () {
                        ...
                        Route::resource('message-settings', 'MessageSettingsController');
                          ...
                          //Chat Settings	
                          Route::get('chat-settings', 'AdminChat@settings')->name('ChatSettings.settings');	
                          Route::post('chat-settings', 'AdminChat@save_settings')->name('ChatSettings.settings-save');
		
                          //Push Notifications Settings	
                          Route::get('push-settings', 'PushNotifications@settings')->name('PushSettings.settings');	
                          Route::post('push-settings', 'PushNotifications@save_settings')->name('PushSettings.settings-save');
                          
                          //Push Notifications Settings	
                          Route::get('files-settings', 'FilesController@settings')->name('FilesSettings.settings');	
                          Route::post('files-settings', 'FilesController@save_settings')->name('FilesSettings.settings-save');
                          
                          //Widget Code Settings
                          Route::get('widget-code', 'AdminWidgetCodeController@settings')->name('WidgetCode.settings');	
                          Route::post('widget-code', 'AdminWidgetCodeController@save_settings')->name('WidgetCode.settings-save');
                      ...
                      Route::get('pixel', ['uses' => 'AdminPixelController@index'])->name('pixel');
                    Route::get('addpixel', ['uses' => 'AdminPixelController@addpixel'])->name('addpixel');
                    Route::post('savepixel', ['uses' => 'AdminPixelController@savepixel'])->name('savepixel');

                    Route::get('editPixel/{articleID}', ['uses' => 'AdminPixelController@editPixel'])->name('editPixel');
                    
                    Route::post('updatepixel}', ['uses' => 'AdminPixelController@updatepixel'])->name('updatepixel');

                    Route::post('deletePixel}', ['uses' => 'AdminPixelController@deletePixel'])->name('deletePixel');
                ...
                Route::group(
                    ...
                    function () {
                        ...
                        Route::resource('milestones', 'MarknageProjectMilestonesController');
                ...
                // Files
                Route::resource('user-file', 'AdminFileController');
                Route::get('fileGroupCategory/create-group', ['uses' => 'ManageFileGroupController@createCat'])->name('fileGroupCategory.create-group');
                Route::post('fileGroupCategory/store-group', ['uses' => 'ManageFileGroupController@storeGroup'])->name('fileGroupCategory.store-group');
                Route::delete('fileGroupCategory/destroy/{id}', ['uses' => 'ManageFileGroupController@destroy'])->name('fileGroupCategory.destroy');    
                Route::post('fileGroupCategory/assign-group', ['uses' => 'ManageFileGroupController@assignGroup'])->name('fileGroupCategory.assign-group');
                Route::delete('fileGroupCategory/delete-group/{id}', ['uses' => 'ManageFileGroupController@deleteGroup'])->name('fileGroupCategory.delete-group');    
                Route::get('fileGroupCategory/manage-files', ['uses' => 'ManageFileGroupController@manageFiles'])->name('fileGroupCategory.manage-files');
                Route::get('fileGroupCategory/add-files', ['uses' => 'ManageFileGroupController@addFiles'])->name('fileGroupCategory.add-files');
                Route::post('fileGroupCategory/store-files', ['uses' => 'ManageFileGroupController@storeFiles'])->name('fileGroupCategory.store-files');
            
            	//Admin Files Without Groups	
            	Route::get('files/manage-files/{token}', ['uses' => 'FilesController@manage_files'])->name('files.manage-files')->middleware('files-limit');	
            	Route::post('files/add-folder', ['uses' => 'FilesController@add_folder'])->name('files.add-folder')->middleware('files-limit');	
            	Route::post('files/add-files', ['uses' => 'FilesController@add_files'])->name('files.add-files')->middleware('files-limit');	
            	Route::post('files/change-user', ['uses' => 'FilesController@change_user'])->name('files.change-user')->middleware('files-limit');	
            	Route::post('files/delete-file', ['uses' => 'FilesController@delete_file'])->name('files.delete-file')->middleware('files-limit');
				Route::post('files/delete-folder', ['uses' => 'FilesController@delete_folder'])->name('files.delete-folder')->middleware('files-limit');	
            	Route::post('files/get-password', ['uses' => 'FilesController@get_password'])->name('files.get-password')->middleware('files-limit');	
            	Route::post('files/set-password', ['uses' => 'FilesController@set_password'])->name('files.set-password')->middleware('files-limit');	
            	Route::get('files/download-file/{token}', ['uses' => 'FilesController@download_file'])->name('files.download-file')->middleware('files-limit');	
            		
            	//Admin Chat Routes	
            	Route::get('chat', 'AdminChat@chat_board')->name('chat.chat-board')->middleware('chat-limit');
            	//Route::get('chat-v2', 'AdminChat@chat_board_v2')->name('chat.chat-board-v2');
            	Route::get('missed-chats', 'AdminChat@missed_chats')->name('chat.missed-chats')->middleware('chat-limit');
            	Route::get('ended-chats', 'AdminChat@ended_chats')->name('chat.ended-chats')->middleware('chat-limit');
            	Route::post('chat/open-chat', 'AdminChat@open_chat')->name('chat.open-chat');	
            	Route::get('chat-settings', 'AdminChat@chat_settings')->name('chat.settings')->middleware('chat-limit');	
            	Route::post('chat-settings', 'AdminChat@update_settings')->name('chat.settings.save');	
            	Route::get('chat/{token}', 'AdminChat@load_chat')->name('chat.load-chat');	
            	Route::post('chat/load-visitors', 'AdminChat@load_visitors')->name('chat.load-visitors');	
            	Route::post('chat/load-chat', 'AdminChat@load_chat')->name('chat.load-chat');	
            	Route::post('chat/load-my-chat', 'AdminChat@load_my_chat')->name('chat.load-my-chat');	
            	Route::post('chat/assign-chat', 'AdminChat@assign_chat')->name('chat.assign-chat');	
            		
            	Route::post('chat/send-msg/{token}', 'AdminChat@send_msg')->name('chat.send-msg');	
            	Route::post('chat/end-chat', 'AdminChat@end_chat')->name('chat.end-chat');	
            		
            	//Public Image Upload Route (Returns URL)	
            	Route::post('image-upload', 'PushNotifications@image_upload')->name('image-upload');	
            	Route::post('icon-upload', 'PushNotifications@icon_upload')->name('icon-upload');	
            		
            	//Admin Push Routes	
            	Route::get('New-Push-Notification', 'PushNotifications@new_notification')->name('push.new-notification')->middleware('push-limit');
            	Route::post('New-Push-Notification', 'PushNotifications@new_notification_save')->name('push.new-notification-save');	
            	Route::get('New-Push-Campaign', 'PushNotifications@new_campaign')->name('push.new-campaign')->middleware('push-limit');	
            	Route::post('New-Push-Campaign', 'PushNotifications@new_campaign_save')->name('push.new-campaign-save');	
            	Route::post('push-load-campaign-preview', 'PushNotifications@load_campaign_preview')->name('push.load-campaign-preview');	
            	Route::post('push-load-preview', 'PushNotifications@load_preview')->name('push.load-preview');	
            	Route::post('push-get-senders', 'PushNotifications@get_senders')->name('push.get-senders');	
            	Route::get('All-Notifications', 'PushNotifications@notifications')->name('push.all-notifications')->middleware('push-limit');	
            	Route::get('Push-Notifications', 'PushNotifications@new_campaign')->name('push.new-campaign')->middleware('push-limit');	
            	Route::get('All-Campaigns', 'PushNotifications@campaigns')->name('push.all-campaigns')->middleware('push-limit');
            	Route::get('subscribers', 'PushNotifications@subscribers')->name('push.subscribers')->middleware('push-limit');
            	Route::post('subscribers', 'PushNotifications@get_subscribers')->name('push.get-subscribers')->middleware('push-limit');
            
            	
            
            // group messages
            
            Route::resource('message-group', 'AdminMessageGroupController');
            Route::get('messageGroup/create-group', ['uses' => 'ManageMessageGroups@createCat'])->name('messageGroup.create-group');
            Route::post('messageGroup/store-group', ['uses' => 'ManageMessageGroups@storeGroup'])->name('messageGroup.store-group');
            Route::delete('messageGroup/destroy/{id}', ['uses' => 'ManageMessageGroups@destroy'])->name('messageGroup.destroy');    
            Route::post('messageGroup/remove-message-group', ['uses' => 'ManageMessageGroups@removeMessageGroup'])->name('messageGroup.remove-message-group');
            
            Route::post('messageGroup/assign-message-group', ['uses' => 'ManageMessageGroups@assignMessageGroup'])->name('messageGroup.assign-message-group');
            Route::post('messageGroup/share-message-group', ['uses' => 'ManageMessageGroups@sendMessageToGroup'])->name('messageGroup.share-message-group');
            Route::get('messageGroup/getMessages', ['uses' => 'ManageMessageGroups@getMessages'])->name('messageGroup.getMessages');
            Route::post('messageGroup/saveMessages', ['uses' => 'ManageMessageGroups@saveMessages'])->name('messageGroup.saveMessages');
            Route::get('messageGroup/showGroupsindex', ['uses' => 'AdminMessageGroupController@showGroupsindex'])->name('messageGroup.showGroupsindex');
            Route::get('messageGroup/group-search', ['uses' => 'ManageMessageGroups@getUserSearch'])->name('messageGroup.group-search');
            
            
            Route::get('group-permission/showMembers/{id}', ['as' => 'group-permission.showMembers', 'uses' => 'AdminMessageGroupController@showMembers']);
            Route::get('group-permission/sendMessage/{id}', ['as' => 'group-permission.sendMessage', 'uses' => 'AdminMessageGroupController@sendMessage']);
            Route::get('group-permission/showMessage/{id}', ['as' => 'group-permission.showMessage', 'uses' => 'AdminMessageGroupController@showMessageToGroup']);
            
            
            
            // Surveys 
            
            Route::get('surveyPages/manageSurveys', ['uses' => 'AdminSurveyPageController@manageSurveys'])->name('surveyPages.manageSurveys')->middleware('surveys-limit');
            
            Route::get('surveyPages/surveyRecord', ['uses' => 'AdminSurveyPageController@surveyRecord'])->name('surveyPages.surveyRecord')->middleware('surveys-limit');
            
            Route::get('surveyPages/surveyList', ['uses' => 'AdminSurveyPageController@surveyList'])->name('surveyPages.surveyList')->middleware('surveys-limit');
            
            Route::post('surveyPages/saveQuestion', ['uses' => 'AdminSurveyPageController@saveQuestion'])->name('surveyPages.saveQuestion')->middleware('surveys-limit');
            
            Route::post('surveyPages/saveDesigns', ['uses' => 'AdminSurveyPageController@saveDesigns'])->name('surveyPages.saveDesigns')->middleware('surveys-limit');
            
            Route::post('surveyPages/saveURL', ['uses' => 'AdminSurveyPageController@saveURL'])->name('surveyPages.saveURL')->middleware('surveys-limit');
            
            Route::delete('surveyPages/delete-survey/{id}', ['uses' => 'AdminSurveyPageController@deleteSurvey'])->name('surveyPages.delete-survey')->middleware('surveys-limit');  
            
            Route::post('surveyPages/show-listings', ['uses' => 'AdminSurveyPageController@showListings'])->name('surveyPages.show-listings')->middleware('surveys-limit');
            
            Route::get('surveyPages/show-responces/{id}', ['uses' => 'AdminSurveyPageController@showResponces'])->name('surveyPages.show-responces')->middleware('surveys-limit');
            ...
            Route::group(['middleware' => ['account-setup']], function () {
                ...
                Route::post('billing/change', 'AdminBillingController@change')->name('billing.change');
                Route::post('billing/get-invoice', 'AdminBillingController@loadInvoice')->name('billing.loadInvoice');
                Route::get('billing/invoice/{id}', 'AdminBillingController@pay_invoice')->name('billing.pay-invoice');
                Route::post('billing/payment', 'AdminBillingController@payment_status')->name('billing.payment-status');
                
                //Pay As You Go
                Route::get('billing/pay-as-you-go', 'AdminBillingController@pay_as_you_go')->name('billing.pay-as-you-go');
                Route::post('billing/pay-as-you-go', 'AdminBillingController@card_save')->name('billing.card_save');
    ...
    Route::group(
        ...
        //endregion
            ...
            Route::post('fileGroupCategory/store-files', ['uses' => 'ManageFileGroupController@storeFiles'])->name('fileGroupCategory.store-files');
              ...
              //Route::get('files/manage-files/{token}', ['uses' => 'FilesController@manage_files'])->name('files.manage-files');	
              //Route::post('files/add-folder', ['uses' => 'FilesController@add_folder'])->name('files.add-folder');	
              /**Route::post('files/add-files', ['uses' => 'FilesController@add_files'])->name('files.add-files');	
              Route::post('files/delete-file', ['uses' => 'FilesController@delete_file'])->name('files.delete-file');	
              Route::post('files/get-password', ['uses' => 'FilesController@get_password'])->name('files.get-password');	
              Route::post('files/set-password', ['uses' => 'FilesController@set_password'])->name('files.set-password');	
              Route::get('files/download-file/{token}', ['uses' => 'FilesController@download_file'])->name('files.download-file');*/


            // Group discussions.

            Route::get('messageGroup/messages', ['uses' => 'ManageGroupMessageController@index'])->name('messageGroup.messages');
            Route::post('messageGroup/saveMessages', ['uses' => 'ManageGroupMessageController@saveMessages'])->name('messageGroup.saveMessages');
            Route::get('messageGroup/getMessages', ['uses' => 'ManageGroupMessageController@getMessages'])->name('messageGroup.getMessages');
